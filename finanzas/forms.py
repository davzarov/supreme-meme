...
class TransferenciaForm(forms.Form):
    ...
    de_cuenta = TransferenciaChoiceField(queryset=Cuenta.objects.all())
    a_cuenta = TransferenciaChoiceField(queryset=Cuenta.objects.all())
    monto = forms.IntegerField(localize=True)

    def __init__(self, *args, **kwargs):
        super(TransferenciaForm, self).__init__(*args, **kwargs)
        self.fields.get('de_cuenta').widget.attrs = {"class": "custom-select"}
        self.fields.get('de_cuenta').empty_label = "Elige una Cuenta..."
        self.fields.get('a_cuenta').widget.attrs = {"class": "custom-select"}
        self.fields.get('a_cuenta').empty_label = "Elige una Cuenta..."
        self.fields.get('monto').widget.attrs = {"class": "form-control"}
...
class AgregarPresupuestoForm(forms.Form):
    presupuesto = forms.CharField(widget=forms.TextInput(attrs={
        "size": "6"
    }))
    monto = forms.IntegerField(localize=True)
    mes_inicio = forms.DateField(widget=forms.TextInput(attrs={
        "class": "",
        "size": "10",
        "data-start-week-day": "0"
    }))
    duracion = forms.ChoiceField(
        widget=forms.RadioSelect, choices=DURACIONES_OPCIONES)
...
class AgregarTransaccionForm(forms.Form):
    ...
    direccion = forms.ChoiceField(
        widget=forms.RadioSelect, choices=DIRECCIONES_OPCIONES, initial="D")
    fecha = forms.DateField(widget=forms.TextInput(attrs={}))
    monto = forms.IntegerField(localize=True)
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['presupuesto'] = forms.ModelChoiceField(
            queryset=Presupuesto.objects.all(), required=False)
